...
- **`selector_miss` extraction status** (`models/enums.py`): HTTP 200, not classified as `blocked`/`captcha`, real page content, but no selector matched a price ⇒ markup drift. Distinct from `extraction_failed` (a selector *did* match but its text would not parse — the markup has not moved, so regenerating would be the wrong remedy). No migration needed: migration `0006` already dropped the `extraction_status` CHECK constraint. Item 15's `classify_block` runs first, unconditionally, so a blocked page can never become a `selector_miss` and poison the stored selector.
- **Per-host `SelectorProfile` store** (`app/models/selector_profile.py`, `SelectorProfileStatus` enum, `app/services/selector_profile_service.py`, Alembic migration `0010`): one row per normalised host (lower-case, `www.` stripped) carrying the validated selector, an in-place `version` counter, generation metadata, and the regeneration state (`failed_attempts` / `last_attempt_at`). Regional storefronts stay distinct (`amazon.co.uk` ≠ `amazon.com` — their markup differs). Selectors are runtime-editable data rather than constants requiring a redeploy.
- **Provider-agnostic generation via Pydantic AI** (`app/services/llm/`): **OpenAI, Anthropic, Azure OpenAI and OpenRouter** are all reachable through one code path, so onboarding a provider is a config change. `output_type=SelectorSuggestion` schema-validates the reply at the framework boundary — no provider-specific response parsing exists anywhere in the app. A single long-lived agent carries the instructions and output type; the model is supplied **per run**, so one agent serves every credential.
- **Two credential scopes, no auth system required** (`app/services/llm/client.resolve_llm_config`): an env-configured **admin default** (`LLM_PROVIDER`/`LLM_MODEL`/`LLM_API_KEY`, plus `AZURE_OPENAI_ENDPOINT`/`AZURE_OPENAI_API_VERSION`) and a per-product **bring-your-own** credential. Resolution order: product BYO → admin default → `None` (generation disabled — the documented state for a deployment with no key, not an error; scrapes still complete and record `selector_miss`).
- **Encrypted per-product BYO credentials** (`app/models/product_llm_credential.py`, `app/core/crypto.py`, `app/services/llm_credential_service.py`, `PUT`/`GET`/`DELETE /api/v1/products/{id}/llm-credential`): keys are Fernet-encrypted at rest (keyed off the already-validated `SECRET_KEY`), decrypted only at generation time, never logged — not even in `__repr__`, which reaches logs — and **never** returned by any endpoint; the read schema exposes provider/model and a masked hint only. A credential written under a rotated `SECRET_KEY` decrypts to `None` and falls back to the admin default rather than 500-ing.
- **Asynchronous validate-then-promote regeneration** (`app/tasks/selector.py`): a `selector_miss` marks the host stale and enqueues regeneration on the `playwright` queue while the existing selectors keep serving. A suggestion is promoted only after it extracts a plausible numeric price from the live page (reusing the shared `_normalize_price_text`), so an `active` profile is always a proven one — model confidence is recorded but never decides. The task never raises and never retries: `SELECTOR_MAX_REGEN_ATTEMPTS` and `SELECTOR_REGEN_COOLDOWN_HOURS` bound the cost, and the cooldown also collapses the N jobs that N products on one host enqueue in the same cycle.
- **`POST /api/v1/products/{id}/report-selector-issue`** (202): marks the host's profile stale and enqueues regeneration. Clears a spent attempt budget — a human asserting the price is wrong is exactly the signal that should revive a host parked as `failed` — while still respecting the cooldown, so repeated reports do not queue duplicate work. The response says whether regeneration was actually enqueued.
- **Drift is separately observable on `GET /api/v1/products/failing`**: a `selector_miss` failure category and a `selector_miss_count` aggregate alongside Item 15's `blocked_count`/`captcha_count` — kept separate because the remedies differ (regenerate selectors vs rotate proxies) — plus a per-item `host`, the unit a heal acts on.
- **Configurable API endpoint** (`LLM_BASE_URL`): routes selector generation through an LLM gateway, an egress proxy, or a self-hosted OpenAI-compatible server instead of the provider's public API. Only `openai` and `anthropic` accept one — OpenRouter's endpoint is fixed and Azure uses `AZURE_OPENAI_ENDPOINT`, so setting it alongside either is **rejected at startup** rather than silently ignored (which would leave traffic on the public API while the operator believed it was routed away). Applies to the admin default only: a per-product BYO key always reaches the provider's real endpoint, so a user's own credential is never sent to infrastructure they did not choose.
- **New settings** (`LLM_PROVIDER`, `LLM_MODEL`, `LLM_API_KEY`, `LLM_BASE_URL`, `AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_API_VERSION`, `SELECTOR_HTML_MAX_BYTES`, `SELECTOR_MAX_REGEN_ATTEMPTS`, `SELECTOR_REGEN_COOLDOWN_HOURS`) documented in `.env.example`, with startup validation for the provider name, the base-URL format and provider compatibility, and Azure's two mutually-exclusive endpoint styles.
- **Opt-in live-provider tests** — the Live E2E layer Item 16's test strategy called for, which nothing else in the suite can cover: every unit and integration selector test substitutes a Pydantic AI `FunctionModel` with `ALLOW_MODEL_REQUESTS = False`, so a wrong model name, a revoked key, a rejecting gateway, or a provider that stops honouring `output_type` are all invisible until production — where the regeneration task swallows them by design. `make test-llm-live` (`backend/tests/live/`) makes one real call and asserts the returned selector extracts the buy-box price past three decoys; `make test-e2e-llm` runs `docs/behaviour/selector_healing.feature` (`@PP-E2E-043…045`) against the live stack: a fixture page with drifted markup records `selector_miss`, a reported issue triggers regeneration, and a fresh scrape of the *same unchanged page* then records `ok`. Asserted entirely through the public API — the `selector_profile` table is never read — so what is proven is the behaviour a user experiences.
- **`live_llm` marker + drifted fixture layout** (`PUT /fixtures/{slug}/layout`): the money-spending scenarios carry `live_llm` *in addition to* `live_api`, and `make test-e2e` now selects `"live_api and not live_llm"`, so widening a marker cannot sweep them into a free suite or into CI.
- **ADR** `docs/decisions/self-healing-selectors.md`.

### Changed (Self-healing selectors — Item 16)

- **`amazon.py` and `generic.py` consume the host's stored selector**, attached by the registry as `BaseScraper.learned_selector`. Extraction order — Amazon: `ld+json` → stored selector → legacy hardcoded list → `selector_miss`; generic: product `css_selector` → stored selector → `selector_miss`. The built-in paths remain the safety net, so the capability is purely additive and the LLM is optional at runtime.
- **A loaded page with no selector match now records `selector_miss` rather than `extraction_failed`** on both paths. Existing `extraction_failed` semantics narrow to "matched but unparseable".

### Fixed (Self-healing selectors — Item 16)

- **Self-healing livelock: a healed host immediately un-healed itself.** A stored selector was only served while its profile was `active`, but every `selector_miss` demotes the host to `stale` — so a scrape that raced a promotion demoted the profile the worker had just promoted, got nothing on the next read, missed again, and demoted it again. Under ordinary scheduled scraping a host could never converge. Found by the live E2E run (promotion logged at 19:45:19, scrapes still recording `selector_miss` at 19:47:12); invisible to every unit and integration test, because none of them run a scrape and a promotion concurrently. `get_active_selector` is now `get_stored_selector` and serves the incumbent whatever the status — status gates *regeneration*, never *serving*. This also stops a `selector_miss` on one product stripping a working selector from every other product on the host, and a spurious user report doing the same to a healthy host.
- **The E2E `scrape-sync` hook had silently stopped mirroring the real scrape.** It held a *copy* of `scrape_product`'s body, written before Item 16, so it never passed the host's healed selector — meaning the harness could promote a selector and then keep recording `selector_miss` forever, unable to observe the behaviour it exists to verify. Both callers now share one `services/scrape_runner.run_scrape`, so the harness necessarily exercises the code production runs.
- **Config tests depended on the developer's `.env`.** `Settings` was constructed without `_env_file=None`, so a machine with a real `LLM_API_KEY` failed the "empty key is the disabled switch" and Azure-provider assertions — a false red that said nothing about the code.

- **`GenericScraper` extracted a matched element's outer HTML instead of its text**, so a class name containing a hyphen or digit (e.g. `.pdp-price`) leaked into the cleaned digits and could yield a *negative* price. Latent for hand-written selectors and a certainty for LLM-generated ones. Selectors that already carry an explicit `::text` / `::attr(...)` pseudo-element are still passed through untouched.
- **`validation_exception_handler` returned 500 instead of 422 for custom request-body validators**: Pydantic v2 puts the raised exception *object* in `ctx`, which `JSONResponse` cannot serialise. `ctx` values are now stringified; the human-readable text was already in `msg`, so nothing is lost.
