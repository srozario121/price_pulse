...
## TODO item 14 — Standardised, Executed E2E Behaviour Specification (BDD) (2026-07-05)

**Ambiguities found**: 8

| Category | Finding | Resolution |
|---|---|---|
| Scope gaps | Original item deferred the execution model to an ADR ("executed BDD vs. traceable spec"); user chose **executed BDD**, which changes the item from docs-only to test-infrastructure | Scenarios executed directly: `pytest-bdd` (backend) + `playwright-bdd` (frontend); the `.feature` files' step definitions ARE the E2E tests |
| Scope gaps / Integration wiring | Executed BDD makes step definitions overlap Item 13's planned hand-written E2E tests — undefined boundary between 13 and 14 | Resolved: **Item 14 owns `.feature` catalogue + step definitions**; **Item 13 owns harness + CI** (e2e compose profile, fixture server, webhook sink, seed data, `make test-e2e`, CI job). Dependency flips: 14's steps now depend on 13's harness — implement together, harness first |
| Scope gaps | `pytest-bdd` and `playwright-bdd` are not present in `backend/pyproject.toml` / `frontend/package.json` | Added tooling tasks: `pytest-bdd` dev dep + feature-discovery config; `playwright-bdd` dev dep + `test:e2e:bdd` script |
| Conflict / overwrite policy | Gherkin runners need to discover `.feature` files, but the spec must "live in `docs/`" | `docs/behaviour/` is canonical; `pytest-bdd` `bdd_features_base_dir` and `playwright-bdd` `defineBddConfig({features})` point there; step defs live in `backend/tests/e2e/steps/` and `frontend/tests/e2e/steps/` |
| External service calls | Real retail sites are non-deterministic, bot-protected, and cannot be made to drop a price to trigger an alert | Fixture HTTP server (compose service, Item 13 harness) serves canned HTML through the real `generic` scraper; a `PUT /fixtures/{slug}/price` control endpoint mutates the served price so alert-trigger scenarios are deterministic |
| Error & edge-case handling | Non-functional scenarios (30-min beat cadence, 24h cooldown, per-domain rate limiting) can't run against wall-clock time | Dedicated e2e compose profile sets `SCRAPE_INTERVAL_MINUTES=1` and tiny `ALERT_COOLDOWN_HOURS` (both existing `Settings`); gated `E2E_TEST_HOOKS=true` control endpoints force-scrape and reset-cooldown so cadence/cooldown resolve in seconds |
| Model/data design | "Public API only" assertions cannot observe notifications — `NotificationLog` has **no** read endpoint (only products/prices/alerts have routes) | Scope addition: new `GET /api/v1/alerts/{alert_id}/notifications` (paginated `NotificationLogRead`) so email/whatsapp stub + webhook deliveries are assertable via the public API; webhook also targets a harness webhook-sink service |
| Test coverage | Catalogue breadth and isolation on a shared live stack unspecified | Full catalogue (happy-path + negative + non-functional), **all executed** via hooks; isolation by **unique per-scenario fixture URLs** (no global reset); assertions **exclusively via public REST/UI**, no DB/Redis peeking from steps |

**Tasks added**: 12 — `pytest-bdd` dep + discovery config; `playwright-bdd` dep + script; feature-discovery pointing at `docs/behaviour/`; author `docs/behaviour/*.feature` catalogue; `docs/behaviour/README.md` (conventions + `PP-E2E-NNN` scheme + traceability table); backend step definitions (public-API assertions, unique fixture URLs); frontend `playwright-bdd` step definitions; `NotificationLogRead` schema + `GET /alerts/{id}/notifications` route + service method; harness-contract verification task against Item 13; ADR for executed-BDD approach; mandatory worktree-first implementation workflow block
**Tasks removed/changed**: 2 — "Decide executed vs. traceable (ADR)" resolved to executed BDD (ADR now records the chosen approach, not the choice itself); "add a traceability note/docstring" upgraded to a `@PP-E2E-NNN` Gherkin-tag convention with a README traceability table
**Documentation changes**: `docs/behaviour/` (create — features + README); `backend/tests/e2e/steps/` (create); `frontend/tests/e2e/steps/` (create); `backend/pyproject.toml` (update — pytest-bdd + config); `frontend/package.json` (update — playwright-bdd + script); `backend/app/schemas/notification.py` (update — `NotificationLogRead`); `backend/app/api/v1/alerts.py` (update — notifications route); `docs/decisions/` (add ADR); `CLAUDE.md` (update — `docs/behaviour/` source of truth, `PP-E2E-NNN`, `make test-e2e`); `CHANGELOG.md` (update at implementation time)
**Key design constraint**: The `.feature` files under `docs/behaviour/` are the single source of truth AND the executed tests — `pytest-bdd`/`playwright-bdd` are pointed at that directory rather than duplicating features into `tests/`. Every behaviour is deterministic-by-hook (fixture price mutation, tiny e2e intervals, gated control endpoints) and asserted only through the public API/UI, so the catalogue is fully enforced without coupling to internal storage. Item 14's executable steps depend on Item 13's harness; the two are implemented together, harness first.

---

## TODO item 13 — E2E Test Harness & CI for the Live Compose Stack (2026-07-05)

**Ambiguities found**: 8

| Category | Finding | Resolution |
|---|---|---|
| Scope gaps / Integration wiring | After the Item 14 dependency flip, Item 13 still described hand-written pytest live_api tests + Playwright specs that now belong to Item 14 — overlapping ownership | Reframed: **Item 13 owns harness + CI + a thin `@smoke` self-check**; Item 14 owns features + step definitions. Item 13 **blocks** Item 14; implement harness first |
| Architecture & data model | How the e2e-only services (fixture-server, webhook-sink) and test env layer onto the existing two compose files was unspecified | **New `docker-compose.e2e.yml` override** (`-f docker-compose.yml -f docker-compose.e2e.yml`); prod compose untouched; adds services + `E2E_TEST_HOOKS`/`SCRAPE_INTERVAL_MINUTES=1`/tiny cooldown; chosen over `profiles:` to keep test services out of the prod file |
| External service calls | Deterministic scrape target + how notifications are observed left open | **Custom in-repo `fixture-server`** (serves canned HTML through the real generic scraper + `PUT /fixtures/{slug}/price` mutation); **off-the-shelf `webhook-sink`** polled over HTTP for webhook-delivery assertions |
| Conflict / overwrite policy | Test-only control surfaces (force-scrape, reset-cooldown) must never affect production | **Env-flag gated, in-process**: a router mounted only when `settings.E2E_TEST_HOOKS is True` (new config field), set true only by `docker-compose.e2e.yml`; endpoints under `/api/v1/_test/`; test asserts the router is absent when the flag is false |
| Error & edge-case handling | Force-scrape execution model (sync vs async) drives determinism vs. real-path coverage | **Sync inline hook** (`scrape-sync` runs the scrape body inline, returns after PriceRecord + alert eval) as default; Item 14 keeps **one** real-cadence scenario using the 1-min beat + bounded polling |
| Scope gaps | `make test-e2e` already exists but runs frontend Playwright against the **dev server** (`:5173`), not compose; no lifecycle/subset targets | Replace it: `make test-e2e` owns up→run→down (backend pytest-bdd + frontend playwright-bdd); add `make test-e2e-smoke` (@smoke subset) and `make e2e-up`/`e2e-down` helpers |
| Integration wiring | CI execution scope/trigger for executed BDD (slower than liveness smoke) unspecified | New `e2e` CI job (`needs: build`) runs `@smoke` on every PR + uploads Playwright report/traces on failure; **full catalogue nightly (`schedule:`) + `workflow_dispatch`**; existing `smoke` liveness job unchanged; Playwright browsers installed in-job |
| Test coverage | Item 13's own thin smoke E2E scope, and how it avoids duplicating Item 14, unspecified | Item 13's self-check = **`@smoke`-tagged subset of Item 14 features** via the BDD runner — so a first `@smoke` slice of Item 14 features must exist; sequencing note added |

**Tasks added**: ~15 — `docker-compose.e2e.yml` overlay; fixture-server app + `docker/fixture-server.Dockerfile`; webhook-sink service; `E2E_TEST_HOOKS` config field + `.env.example` note; gated `_test_hooks.py` router + conditional include in `main.py` + absence-when-false guard; `scrape-sync` + `reset-cooldown` endpoints; replace `make test-e2e` and add `e2e-up`/`e2e-down`/`test-e2e-smoke`; `playwright.config.ts` compose base URL; CI `e2e` job (@smoke on PR); nightly/manual full-catalogue trigger; `@smoke` self-check acceptance task
**Tasks removed/changed**: 3 — the backend `@pytest.mark.live_api` pipeline tests and the Playwright behaviour specs moved to Item 14 (13 keeps only the harness + `@smoke` self-check); "add `test:e2e`/`test:e2e:ci` scripts" folded into the `make` lifecycle + playwright-bdd (Item 14); the single CI-job task split into per-PR `@smoke` + nightly full
**Documentation changes**: `docker-compose.e2e.yml` (create); `docker/fixture-server.Dockerfile` (create); `tests/e2e/fixture_server/` (create); `backend/app/core/config.py` (update — `E2E_TEST_HOOKS`); `backend/app/api/v1/_test_hooks.py` (create); `backend/app/main.py` (update — conditional router); `.env.example` (update); `frontend/playwright.config.ts` (update); `Makefile` (update — lifecycle targets); `.github/workflows/ci.yml` (update — e2e job + nightly); `CLAUDE.md` (update); `CHANGELOG.md` (update at implementation time)
**Key design constraint**: The e2e concerns are layered as a **separate `docker-compose.e2e.yml` override** and an **env-flag-gated in-process test router** — the production compose file and the app's production route table are never touched, so no test-only service, hook, or env can leak into a non-e2e environment. Item 13 is a hard prerequisite for Item 14, and its own `@smoke` self-check consumes Item 14's `@smoke` feature slice, so the two items are delivered together, harness first.

## TODO item 15 — Anti-Blocking: Rotating Residential Proxies, Realistic UA/Headers & Stealth Context (2026-07-15)

**Ambiguities found**: 11

| Category | Finding | Resolution |
|---|---|---|
| External service calls | Proxy provider + rotation model was open (managed vs BYO; per-request vs sticky) | BYO list in `Settings` (comma-separated env like `CORS_ORIGINS`), per-request rotation, rotate-on-block; no managed provider/sticky state |
| Error & edge-case handling | Block-retry behaviour + budget undefined | Rotate proxy + bounded retry (`MAX_PROXY_ROTATIONS=2`); on exhaustion persist `BLOCKED`/`CAPTCHA`; dead proxy rotates without consuming the block budget |
| Model/data design | Whether to add a distinct `BLOCKED`/`CAPTCHA` `ExtractionStatus` (and migration) | Add both `BLOCKED` and `CAPTCHA`; **no Alembic migration** — `extraction_status` is a plain `Mapped[str]` String column (verified), so values are app-level `StrEnum` additions only |
| Scope gaps | Stealth: `playwright-stealth` dependency vs hand-rolled init scripts | Both — `playwright-stealth` base + custom `add_init_script()` top-ups; add the dep to `backend/pyproject.toml` |
| Scope gaps | Config location for proxy/UA pools (Settings vs external file) | `Settings`-based now; external hot-updatable file deferred to a future item (aligned with Item 16) |
| Scope gaps | UA pool already exists in `http_client._USER_AGENTS` but Playwright path uses stock UA — duplication risk | Consolidate into one shared `scrapers/anti_blocking.py` used by both paths; matched `Accept-Language`/`Sec-CH-UA*` headers |
| Error & edge-case handling | Block detection was framed as status-code only; 200-status CAPTCHA pages mis-classify as `extraction_failed` | Shared `classify_block(status, html)` runs in **both** fetch paths, catching robot-check HTML on HTTP 200 |
| Integration wiring | httpx vs Playwright proxy config shapes differ (`proxy=` string vs dict) | Single normaliser in the shared module emits both shapes from one BYO entry |
| Integration wiring | Monitoring surfacing of the new statuses undecided | Extend `find_failing_products` + `FailingProductRead` + `/products/failing` now to break out blocked/captcha category + counts; aggregate drift monitor stays Item 16 |
| Conflict / overwrite policy | Task 7 "respect robots.txt and ToS" implied new enforcement over existing log-and-proceed | Keep log-and-proceed unchanged; reword task 7 to "extend existing rate-limit/back-off"; enforcement deferred |
| Scope gaps | "Optional per-domain overrides" ambiguous — in scope or not | Out of scope this item; global pools only; per-domain overrides deferred |

**Tasks added**: ~15 — `PROXY_URLS`/`MAX_PROXY_ROTATIONS` Settings + `.env.example`; create `scrapers/anti_blocking.py` (UA/header pool, proxy selector/rotator + normaliser); `classify_block` classifier; `BLOCKED`/`CAPTCHA` enum values (+ CHECK-constraint confirmation); httpx proxy wiring + block-rotate-retry + dead-proxy handling; Playwright context proxy/UA/headers + `playwright-stealth` + custom init scripts + page-level block detection; add `playwright-stealth` dep; extend monitoring service/schema/endpoint for block breakdown; local forward-proxy stub integration fixture; the four-layer test matrix; ADR under `docs/decisions/`
**Tasks removed/changed**: 2 — task 7 reworded from "respect robots.txt/ToS" (implied enforcement) to "extend existing rate-limit/back-off, robots.txt stays log-and-proceed"; the generic "rotate proxy per request (or on block)" split into explicit per-request selection + bounded rotate-on-block retry with a budget
**Documentation changes**: `core/config.py` + `.env.example` (update); `scrapers/anti_blocking.py` (create); `scrapers/http_client.py` + `scrapers/amazon.py` (update); `models/enums.py` (update); `services/monitoring_service.py` + `schemas/product.py` + `api/v1/products.py` (update); `backend/pyproject.toml` (update — `playwright-stealth`); `docs/decisions/` (create ADR); `CLAUDE.md` (update — env table + statuses); `CHANGELOG.md` (update at implementation time)
**Key design constraint**: All anti-blocking mechanics converge in one shared `scrapers/anti_blocking.py` (proxy selection/rotation/normalisation, UA+matched-header pool, `classify_block`) consumed identically by both the httpx and Playwright fetch paths — block detection runs on content as well as status so 200-status CAPTCHA pages are caught, rotation is per-request with a bounded rotate-on-block retry, and the new `BLOCKED`/`CAPTCHA` statuses need no DB migration because `extraction_status` is a plain String column.

## TODO item 16 — Handle Selector Drift → LLM-Generated Self-Healing Selectors (2026-07-15)

**Ambiguities found**: 12 (item was substantially reframed by the user from "maintain hardcoded/externalised selector lists" to "Claude generates the selector on drift")

| Category | Finding | Resolution |
|---|---|---|
| Scope gaps | Whether to hand-maintain selectors or generate them | LLM-generated self-healing selectors: on a loaded page with no price, Claude generates a CSS selector from trimmed HTML, validated + cached per host, reused with no further LLM calls |
| External service calls | What Claude receives to generate a selector | Trimmed HTML (strip scripts/styles/svg, cap `SELECTOR_HTML_MAX_BYTES`) + structured output via `messages.parse()` — schema `{price_selector, currency_selector?, confidence}` |
| Model/data design | Config format/location for selectors (original open decision) | DB-backed `SelectorProfile` table keyed by **host**, versioned, runtime-editable (+ Alembic migration); supersedes the "externalise to config file" idea; per-host because amazon.co.uk ≠ amazon.com markup |
| Integration wiring | How the app authenticates to Anthropic when running (user-flagged) | `ANTHROPIC_API_KEY` + `ANTHROPIC_MODEL` (default `claude-opus-4-8`) in `Settings`/env, injected into backend + celery-playwright workers; bare `AsyncAnthropic()` reads it; documented in `.env.example`, never committed |
| External service calls | Model tier + call shape | Single `messages.parse()` call, modest `max_tokens`, no agent loop/tools; `ANTHROPIC_MODEL` swappable to a cheaper tier; generation infrequent (once per host, then cached) |
| Error & edge-case handling | How (re)generation runs relative to the scrape | Async Celery task off the scrape path; validate (extract a plausible price via `_normalize_price_text`) → persist → promote; bounded attempts + per-host cooldown |
| Error & edge-case handling | What triggers regeneration + drift thresholds (original open decision) | First `selector_miss` for a host marks profile stale + enqueues (cooldown-gated); user report does the same; aggregate window/threshold not needed given cooldown + bounded attempts |
| Model/data design | Distinct `selector_miss` outcome | `SELECTOR_MISS` added to `ExtractionStatus` — HTTP 200, not blocked (Item 15), title present, no price; no DB migration (string column) |
| Conflict / overwrite policy | Interplay with Item 15 blocks | `classify_block` runs first; a `BLOCKED`/`CAPTCHA` page is never `selector_miss` and never triggers generation (would poison the selector) |
| Scope gaps | Which scrapers use LLM selectors + behaviour without a key | Amazon + generic; graceful fallback to today's behaviour (hardcoded Amazon list / product `css_selector`) + `selector_miss` when key unset or generation fails — never crash; `anthropic` dep optional at runtime |
| Integration wiring | User "reports an issue" surface | `POST /api/v1/products/{id}/report-selector-issue` → flag host profile stale, enqueue regeneration (cooldown-respecting), 202 |
| Documentation | Drift monitoring surfacing | Extend `monitoring_service`/`FailingProductRead`/`/products/failing` to break out `selector_miss` per host/source_type (co-ordinate with Item 15) |

**Tasks added**: ~16 — `ANTHROPIC_API_KEY`/`ANTHROPIC_MODEL`/`SELECTOR_*` settings + `.env.example`; `anthropic` dep; `SelectorProfile` model + Alembic migration + store helpers; `SELECTOR_MISS` enum value; `services/selector_generation.py` (trim-HTML + `messages.parse()` + validation); stored-selector extraction layer in `amazon.py` and `generic.py` gated on Item 15's block classifier; async regeneration Celery task (bounded + cooldown); `report-selector-issue` endpoint; `/products/failing` selector_miss breakdown; golden-HTML fixtures + mocked-Anthropic tests; ADR
**Tasks removed/changed**: 3 — "externalise selector lists to a config file" replaced by DB-backed `SelectorProfile`; "periodic canary scrape" descoped to a future safety net (trigger is now first `selector_miss` + user report); "layered fallback chain (ld+json → DOM → embedded state → regex)" recast as `ld+json → stored LLM selector → legacy hardcoded list → selector_miss`
**Documentation changes**: `core/config.py` + `.env.example` (update); `backend/pyproject.toml` (update — `anthropic`); `models/` + Alembic (create — `SelectorProfile` + migration); `models/enums.py` (update); `services/selector_generation.py` (create); `scrapers/amazon.py` + `scrapers/generic.py` (update); `tasks/` (create — regeneration task); `api/v1/products.py` + `schemas/product.py` + `services/monitoring_service.py` (update); `docs/decisions/` (create ADR); `CLAUDE.md` (update — env table + statuses/endpoint); `CHANGELOG.md` (update at implementation time)
**Key design constraint**: Selector drift self-heals via a single structured Claude call (`messages.parse()` over trimmed HTML) whose output is validated against the live page before promotion and cached in a per-host `SelectorProfile`, so subsequent scrapes cost no tokens; the whole path is gated behind Item 15's block classifier (a blocked page never generates), degrades gracefully to the existing hardcoded/`css_selector` behaviour when `ANTHROPIC_API_KEY` is absent or generation fails, and regenerates asynchronously (bounded attempts + per-host cooldown) on the first `selector_miss` or a user report.

---

## TODO item 17 — Queued-Scrape Visibility: List Queued/Running Jobs & Their Statuses (2026-07-15)

**Ambiguities found**: 11 (all three original "Design decisions (open)" items resolved, plus the wiring/edge-case gaps the plain task list left implicit)

| Category | Finding | Resolution |
|---|---|---|
| Model/data design | Source of truth open (durable `ScrapeJob` table vs ephemeral Celery result backend) | New persisted `ScrapeJob` table — durable, indexable, survives Redis flush, rich filtering; result backend rejected (TTL'd, unfilterable) |
| Scope gaps | `scrape_product` is dispatched from **two** paths — on-demand API (`prices.py::trigger_scrape`) **and** the RedBeat beat scheduler (`schedule.py`, worker-side, no API code) — but task 1 only reasoned about the API response | Track **both** via Celery signals; the scheduled 30-min cadence (the bulk of scrapes) must be visible or the Jobs view is empty of exactly the failures it exists to show |
| Integration wiring | Where to create vs transition the row given two producers and one consumer | Producer/consumer split: `before_task_publish` (fires in API **and** beat processes) creates the `queued` row; `task_prerun`/`task_postrun` (worker) transition it via the `state` arg — closes the producer→consumer gap |
| Integration wiring | Signal handlers write async DB, but the aio worker pool means `task_prerun`/`postrun` fire inside a running event loop — `schedule.py`'s `asyncio.run()` pattern would raise `RuntimeError` | Handlers use a dedicated **synchronous** SQLAlchemy session for `ScrapeJob` writes (loop-reentrancy-safe); `asyncio.run()` only works in `worker_ready` because that fires before the loop processes tasks |
| Error & edge-case handling | A task can be Celery `SUCCESS` yet extract no price (`http_error`/`selector_miss`/`blocked`) — status meaning undefined | User chose to **fold** extraction into status: `success` only when `extraction_status == "ok"`; any non-ok outcome or raised task → `failure`; raw retval kept in `extraction_status` + exception in `detail` so "errored" vs "no price" stays distinguishable |
| Error & edge-case handling | `scrape_product` has `acks_late=True` + `max_retries=3`; retries/redelivery re-fire signals with the same `task_id` → duplicate rows | Idempotent upsert keyed by unique `task_id`; `retries` bumped from `task.request.retries`; exactly one row per logical job |
| Conflict / overwrite policy | Product deleted between schedule-fire and publish → FK insert would fail | Best-effort create: caught + logged + skipped; dispatch never fails (mirrors existing "never fail the request on error" guards) |
| Error & edge-case handling | A bug in job tracking could break scraping/notifications | Every handler filters to `scrape_product` and is fully guarded (log-and-continue, never raise); observability is strictly additive |
| Scope gaps | Retention/pruning policy open — 30-min cadence × every product grows the table unbounded | Periodic `prune_scrape_jobs` Celery beat task deletes rows older than new `SCRAPE_JOB_RETENTION_DAYS` (default 7), daily |
| Scope gaps | "Native app view vs Flower" + live queue depth open | Native durable table + `/scrape-jobs` API is the core; live `queue-depth` via Celery `inspect` marked **optional/best-effort** (aio-pool inspect reliability uncertain), deferrable without blocking the item |
| Test coverage | Test strategy had only 3 layers (unit/integration/live) — no dedicated **negative** layer, the plan-review four-layer requirement | Added full negative layer (unknown-product 404, `limit=101`→422, deleted-product publish, guarded handler DB failure, retries-exhausted → `failure` not stuck `started`, `queue-depth` with no worker) |

**Tasks added**: ~16 — `ScrapeJobStatus` enum; `ScrapeJob` model + Alembic migration; `SCRAPE_JOB_RETENTION_DAYS` setting; `workers/scrape_job_signals.py` (sync-session handlers for both dispatch paths); `trigger="on_demand"` header from the API call-site; `GET /scrape-jobs` + `GET /products/{id}/scrape-jobs` + `ScrapeJobRead` schema + router registration; optional `queue-depth` endpoint; `prune_scrape_jobs` task + daily beat entry; frontend Jobs view + per-product status badge + `useScrapeJobs` hook + `api/client.ts`/types + MSW handlers; ADR; the four-layer test matrix incl. a new negative layer; the mandatory worktree-first Implementation workflow block
**Tasks removed/changed**: 2 — task 1 ("on dispatch create a `ScrapeJob` … update via signals **or** the result backend") narrowed to signal-driven lifecycle across both dispatch paths with an explicit producer/consumer split and sync-session requirement; the reconciliation task recast as "`task_id` is the join key" with a `task_id` filter on the list endpoint (no change to the existing `ScrapeJobResponse` contract)
**Documentation changes**: `models/enums.py` (update — `ScrapeJobStatus`); `models/scrape_job.py` (create); `alembic/versions/` (create — migration); `core/config.py` + `.env.example` (update — `SCRAPE_JOB_RETENTION_DAYS`); `workers/scrape_job_signals.py` (create); `workers/celery_app.py` (update — import signals); `api/v1/prices.py` (update — `on_demand` header); `api/v1/scrape_jobs.py` (create) + `api/v1/router.py` (update); `schemas/scrape_job.py` (create); `tasks/maintenance.py` (create — prune task); `frontend/src/` (create/update — Jobs view, badge, hook, client, types, MSW); `docs/decisions/` (create ADR); `CLAUDE.md` (update — API section, env table); `CHANGELOG.md` (update at implementation time)
**Key design constraint**: The `ScrapeJob` lifecycle is driven by Celery signals with a strict producer/consumer split — `before_task_publish` (which fires in both the API and beat processes) creates the `queued` row so scheduled *and* on-demand scrapes are captured uniformly, while `task_prerun`/`task_postrun` transition it worker-side; because the worker runs the `celery-aio-pool` those per-task signals fire inside a running event loop, so all `ScrapeJob` writes go through a dedicated **synchronous** session, are idempotently upserted by unique `task_id` (surviving `acks_late` retries/redelivery), fold the extraction outcome into a single `success`/`failure` status while retaining the raw `extraction_status`/`detail`, and are fully guarded so a tracking bug can never break the scrape or notification pipeline.

---

## TODO item 18 — Configurable Monitoring Sources: eBay, Currys, John Lewis & Facebook Marketplace Presets (UK) (2026-07-16)

**Ambiguities found**: 9 (all six "Open design questions" resolved, plus three gaps surfaced during codebase exploration)

| Category | Finding | Resolution |
|---|---|---|
| Model/data design | "Configurable from the application" — which layer owns the presets (open decision) | DB-backed `SourcePreset` table (key, label, host_patterns, strategy, default selectors, queue, enabled, version) + Alembic migration + built-in seed; runtime-editable, mirroring Item 16's `SelectorProfile`; config-file/`Settings` options rejected |
| Model/data design | `product.source_type` is a **native** Postgres enum — every new source is an `ALTER TYPE … ADD VALUE` (open decision) | Migrate `source_type_enum` → validated `String` column (matches the `extraction_status` string convention, Items 15–17); prerequisite for data-driven presets; registry is the validation authority |
| Scope gaps | **Two divergent `SourceType` enums** exist — `models/product.py` (native-enum, lowercase) and `scrapers/registry.py` (uppercase `StrEnum`) — both can drift, neither is authoritative | Remove both; `get_scraper`/`queue_for_source_type`/schema validation all resolve from the single DB-backed preset registry |
| Conflict / overwrite policy | Auto-detect `source_type` from URL host vs keep explicit (open decision) | Keep `source_type` explicit on `POST /products`; presets only **validate** it (unknown/disabled → 422); no host-matching inference resolver (smaller scope, no ambiguity rules) |
| Integration wiring | Which new sources need the `playwright` queue — hardcoded `frozenset({AMAZON})` today (open decision) | Queue becomes preset-carried/data-driven: eBay→`default` (httpx+`ld+json`); Currys/John Lewis→`playwright` (React/SPA); FB Marketplace→`playwright`; `queue_for_source_type` reads the preset |
| External service calls | Facebook Marketplace feasibility — login-walled, bot-protected, no `ld+json` (open sub-decision) | User chose a **full** scraper this item; hard-gated on **Item 15 merged first** (reuses its anti-blocking module + `BLOCKED`/`CAPTCHA` statuses so the login wall never reads as `extraction_failed`); ships enabled; ToS/robots/auth risk captured in the ADR |
| Test coverage | Definition of done per scraper — fixtures only vs live check (open decision) | All four layers: golden-HTML unit fixtures + real-DB integration + negative (disabled key→422, FB login-wall→blocked/captcha) + one gated `@pytest.mark.live_api` scrape per retailer (opt-in, excluded from CI) |
| Scope gaps | Deep-research catalogue timing — run now vs during implementation vs follow-up | Authored **in-worktree during implementation**, committed with the scrapers as `docs/research/uk-ecommerce-sources.md` |
| Model/data design | `css_selector_currency` exists on the `Product` model + generic scraper but is **absent** from `ProductBase`/`ProductCreate` schemas — unreachable via the API | Add `css_selector_currency` to the product schemas (presets now carry `default_css_selector_currency`) |

**Tasks added**: ~14 — `SourcePreset` model + `alembic/env.py` registration; `source_preset` table + 6-preset seed migration; `source_preset_service` (resolve + validate); `source_type_enum`→`String` migration; remove both `SourceType` enums; string `source_type` validation + `css_selector_currency` in schemas; data-driven `get_scraper`/`queue_for_source_type`; `GET /api/v1/sources` + `SourcePresetRead` + router registration; `ebay.py`/`currys.py`/`john_lewis.py` scrapers; `facebook_marketplace.py` (Item-15-gated); deep-research doc; `/products/failing` per-source attribution; frontend dropdown + MSW; ADR; the four-layer test matrix
**Tasks removed/changed**: 3 — "DB table **or** config file" narrowed to DB-backed `SourcePreset`; "migrate off enum **or** ADD VALUE" resolved to the String migration; FB Marketplace "**only if** feasibility approves, else ship disabled" replaced by "full scraper, gated on Item 15 merged, ships enabled"
**Documentation changes**: `models/source_preset.py` (create); `services/source_preset_service.py` (create); `schemas/source_preset.py` (create); `scrapers/registry.py` (update — data-driven, remove enum/`_REGISTRY`/`_PLAYWRIGHT_SOURCE_TYPES`); `scrapers/ebay.py`+`currys.py`+`john_lewis.py`+`facebook_marketplace.py` (create); `models/product.py` (update — String column); `schemas/product.py` (update — validation + `css_selector_currency`); `alembic/versions/` (create — 2 migrations); `api/v1/sources.py`+`router.py` (create/update); `frontend/src/` + `tests/mocks/handlers.ts` (update); `docs/research/uk-ecommerce-sources.md` (create); `docs/decisions/` (create ADR); `CLAUDE.md` (update); `CHANGELOG.md` (update at implementation time — Added + Changed)
**Key design constraint**: Monitoring sources become data-driven through a single DB-backed `SourcePreset` registry that is the sole authority for the valid/enabled `source_type` set, its extraction strategy, and its Celery queue — which forces `product.source_type` off the native Postgres enum onto a validated `String` column, collapses the two divergent `SourceType` enums into one, and turns onboarding a UK retailer into a data change; the three retailer scrapers (eBay httpx+`ld+json`, Currys/John Lewis Playwright) are independent, while the full Facebook Marketplace scraper is hard-gated on Item 15 being merged first so its login-wall/bot-check classifies as `blocked`/`captcha` on the shared anti-blocking module rather than silently degrading to `extraction_failed`.
