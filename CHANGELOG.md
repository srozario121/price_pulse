...
- **New settings** (`LLM_PROVIDER`, `LLM_MODEL`, `LLM_API_KEY`, `AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_API_VERSION`, `SELECTOR_HTML_MAX_BYTES`, `SELECTOR_MAX_REGEN_ATTEMPTS`, `SELECTOR_REGEN_COOLDOWN_HOURS`) documented in `.env.example`, with startup validation for the provider name and for Azure's two mutually-exclusive endpoint styles.
