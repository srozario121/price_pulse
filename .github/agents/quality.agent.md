...
5. **Create actionable todos** — after the quality run, add a todo item for each failed gate or priority fix area. Use the format: `Fix <gate>: <brief issue> — see logs/quality/<timestamp>/report.md`

## Todo Creation Rules

After the quality run completes, if any gate fails, create todos following these rules:

- One todo per **distinct failure category**
- For test failures: list the failing test file paths
- For type errors: list the source files with errors
- For coverage gaps: list the files below the threshold
- For code quality: note which thresholds were exceeded
- Example todos:
  ```
  Fix backend_tests: 2 failures in unit/test_price_service.py — see logs/quality/<ts>/report.md
  Fix typecheck: 5 errors in app/services/alert_service.py — see logs/quality/<ts>/report.md
  Improve coverage: app/scrapers/amazon.py at 42% — see logs/quality/<ts>/report.md
  Fix code_quality: CC P95=9.2 exceeds threshold of 7 — see logs/quality/<ts>/report.md
  Fix frontend_tests: 3 failures in Dashboard.test.tsx — see logs/quality/<ts>/report.md
  ```
